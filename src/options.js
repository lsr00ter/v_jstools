...
document.getElementById("myenc").addEventListener("click", function (e) {
  try {
    ...
    var key = CryptoJS.enc.Utf8.parse(md5.slice(0, 16)); //16 位
...
function response_changer_init(idname, titlenames, init_data, callback) {
  ...
  title += "</tr>";
  title +=
    '<tr><td colspan="5"><button>添加行</button><button>添加示例 1(重定向到本地文件)</button><button>添加示例 2(直接修改返回值)</button></td></tr>';
  $(cid).html(title);
...
normal_config.addEventListener("click", function (e) {
  ...
  document.querySelectorAll("input").forEach(function (v) {
    if (
      openlist.indexOf(v.dataset.key) != -1 ||
